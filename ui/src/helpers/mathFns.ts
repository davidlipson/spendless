export const getRange = (total: number, budget: number) => {
    if ((100 * total) / budget < 75) {
        return { class: 'spendlo-ext-below-budget', colour: '#679093' };
    } else if ((100 * total) / budget < 100) {
        return { class: 'spendlo-ext-approaching-budget', colour: '#DCC7B4' };
    } else {
        return { class: 'spendlo-ext-above-budget', colour: '#D98C74' };
    }
};
